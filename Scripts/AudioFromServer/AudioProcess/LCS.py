        ...
        with open(self.DirectPath + "/CosineSimilarityResult.csv") as CosineFile:
            ...
            for row in reader:
                ...
                if float(row[2]) >= maxCos/2:
                    DocNum = str(row[1])
                    DocIDfinder = re.findall("\d\d\.txt", DocNum)
                    DocIDfinder = str(DocIDfinder)
                    DocIDfinder = DocIDfinder.replace("[", "")
                    DocIDfinder = DocIDfinder.replace("[", "")
                    DocIDfinder = DocIDfinder.replace("'", "")
                    DocIDfinder = DocIDfinder.replace(".txt", "")
                    DocID = DocIDfinder
    ...
    def VideoCutter(self):
        ...
        for file in path:
            ...
            if file.endswith(VideoExtension):
            	print(file)
            	ffmpeg_extract_subclip(file, start, end, targetname="SegmentedVideo.MP4")
